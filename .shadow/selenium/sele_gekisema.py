...
def main_loop(first_pos):
    ...
    while True:
        ...
        print('pos:{0}, state:{1}, sma13:{2:10.7f}, sma21:{3:10.7f}'.format(pos, state, sma13[-1], sma21[-1])) ## debug ##

        if pos:
            if state == -1:
                if eval('pre_Close' + test + 'sma13[-1]'):
                    state = -3
                elif eval('pre_Close' + test2 + 'sma21[-1]'):
                    state = -3
            ...
            if state < 0:
                ...
                if pre_Close > sma13[-1] > sma21[-1]: # long
                    state = 1; button = button_ask; test = '>'; test2 = '<'; color = C.R; info = info_ask_avg_price;
                elif pre_Close < sma13[-1] < sma21[-1]: # short
                    state = 1; button = button_bid; test = '<'; test2 = '>'; color = C.B; info = info_bid_avg_price;
                else:
                    pass
