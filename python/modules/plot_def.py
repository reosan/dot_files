...
def plot_ohlc(args, close, ax, tohlc, SPAN_s, SPAN_m, SPAN_l, SPAN_ll, HL_BAND_PERIOD):
    if len(args) >= 3 and args[2] == 'ohlc':
        ...
        pd.Series(close).rolling(SPAN_s).mean().plot(style='y', label=str(SPAN_s), use_index=False)
        pd.Series(close).rolling(SPAN_m).mean().plot(style='c', label=str(SPAN_m), use_index=False)
